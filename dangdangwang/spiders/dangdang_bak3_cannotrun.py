...
class DangdangSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for link in le.extract_links(response):
            ...
            if first_category == '计算机/网络':
                yield Request(link.url, callback=self.parse_second, meta={'first_category': first_category})

            # yield Request(link.url, callback=self.parse_second, meta={'first_category': first_category})

    def parse_subname(self, response, meta_data):
        print('parse_subname scrapy the subname url:{},meta:{}'.format(response.url, meta_data))
        self.logger.debug('parse_subname scrapy the subname url:{},meta:{}'.format(response.url, meta_data))
        if '价格' in response.xpath('//*[@id="navigation"]/ul/li/@dd_name').extract():
            ...
            for link in le.extract_links(response):
                yield Request(link.url, callback=self.parse_books, meta=meta_data)
        else:
    ...
    def parse_second(self, response):
        ...
        second_category = ''
        meta_data = {'first_category': first_category, 'second_category': second_category}
        ...
        if response.xpath('//*[@id="navigation"]/ul/li[1]/div[1]/text()').extract_first() == '分类':
            ...
            self.parse_subname(response, meta_data)
    ...
    def parse_forth(self, response):
        ...
        if response.xpath('//*[@id="navigation"]/ul/li[1]/div[1]/text()').extract_first() == '分类':
            ...
            print('parse_forth scrapy the subname url:{},meta:{}'.format(response.url, meta_data))
            self.logger.debug('parse_forth scrapy the subname url:{},meta:{}'.format(response.url, meta_data))
            self.parse_subname(response, meta_data)
