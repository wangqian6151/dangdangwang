...
class DangdangSpider(scrapy.Spider):
    ...
    def parse_third(self, response):
        ...
        if '分类' in response.xpath('//*[@id="navigation"]/ul/li/@dd_name').extract():
            ...
            le = LinkExtractor(restrict_css='.filtrate_list')
            for link in le.extract_links(response):
                print('parse_third else url:{},text{}'.format(link.url, link.text))
                self.logger.debug('parse_third else url:{},text{}'.format(link.url, link.text))
                yield Request(link.url, callback=self.parse_books,
                              meta={'first_category': first_category, 'second_category': second_category,
                                    'third_category': third_category, 'forth_category': forth_category})
    ...
    def parse_forth(self, response):
        ...
        if '分类' in response.xpath('//*[@id="navigation"]/ul/li/@dd_name').extract():
            ...
            le = LinkExtractor(restrict_css='.filtrate_list')
            for link in le.extract_links(response):
                print('parse_forth else url:{},text{}'.format(link.url, link.text))
                self.logger.debug('parse_forth else url:{},text{}'.format(link.url, link.text))
                yield Request(link.url, callback=self.parse_books,
                              meta={'first_category': first_category, 'second_category': second_category,
                                    'third_category': third_category, 'forth_category': forth_category})
    ...
    def parse_fifth(self, response):
        ...
        le = LinkExtractor(restrict_css='.filtrate_list')
        for link in le.extract_links(response):
            print('parse_fifth else url:{},text{}'.format(link.url, link.text))
            self.logger.debug('parse_fifth else url:{},text{}'.format(link.url, link.text))
            yield Request(link.url, callback=self.parse_books,
                          meta={'first_category': first_category, 'second_category': second_category,
                                'third_category': third_category, 'forth_category': forth_category})
